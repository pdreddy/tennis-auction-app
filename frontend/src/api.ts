...
function normalize(doc: any) {
  if (!doc) return null;
  const teams = toArray(doc.teams).map((t: any) => ({
    ...t,
    players: toArray(t.players),
  }));
  ...
  if (Array.isArray(cb))
    cb = Object.fromEntries(
      cb.map((v: any, i: number) => [String(i), v]).filter(([, v]) => v),
    );
...
function effective(doc: any) {
  ...
  if (effPool >= POOL_ORDER.length)
    return { effPool, effPlayer: 0, poolKey: null, player: null };
  const pool = pools[POOL_ORDER[effPool]] || [];
  const effPlayer =
    effPool === doc.currentPoolIndex
...
function validateBid(doc: any, teamId: number, amount: number): string | null {
  ...
  if (team.players.length >= TEAM_SIZE)
    return `Team full (${TEAM_SIZE}/${TEAM_SIZE})`;
  const utr = getUTRFromKey(poolKey!);
  const fromPool = team.players
    .slice(1)
    .filter((p: any) => p.utr === utr).length;
  const cap = doc.poolCaps?.[poolKey!] ?? buildPoolCaps()[poolKey!] ?? 0;
  ...
  const duplicate = doc.teams.find(
    (t: any) =>
      t.id !== teamId && (doc.currentBids[String(t.id)] ?? 0) === amount,
  );
  if (duplicate)
    return `$${amount.toLocaleString()} taken by ${duplicate.name} — bid higher`;
  ...
  if (amount > team.budget - minNeeded)
    return `Need $${minNeeded.toLocaleString()} for ${remaining - 1} more`;
  return null;
}

async function loadAuctionConfig(): Promise<AuctionConfig> {
  await ensureSeeded();
  const snap = await get(ref(db, "auctionConfig/default"));
  if (!snap.exists()) return DEFAULT_AUCTION_CONFIG;
  const config = snap.val();
  return {
    players: toArray(config.players),
    teams: toArray(config.teams),
  };
}

async function initialDoc(sid: string) {
  const auctionConfig = await loadAuctionConfig();
  return {
    sessionId: sid,
    configSource: "auctionConfig/default",
    teams: getInitialTeams(auctionConfig),
    playerPools: freshPools(auctionConfig),
    poolCaps: buildPoolCaps(auctionConfig),
    currentPoolIndex: 0,
...
export async function ensureSeeded(): Promise<void> {
  ...
  seedPromise = (async () => {
    ...
    const configSnap = await get(ref(db, "auctionConfig/default"));
    if (!configSnap.exists()) {
      updates["auctionConfig/default"] = DEFAULT_AUCTION_CONFIG;
    }

    const adminSnap = await get(ref(db, "users/ADMIN"));
    if (adminSnap.exists()) {
...
export const api = {
  ...
  config: async (_token: string) => configSummary(await loadAuctionConfig()),

  createAuction: async (_token: string) => {
    let sid = newSessionId();
    while ((await get(ref(db, `auctions/${sid}`))).exists())
      sid = newSessionId();
    await set(ref(db, `auctions/${sid}`), await initialDoc(sid));
  ...
  getAuction: async (sid: string, _token: string) => {
    ...
    if (!snap.exists())
      throw { status: 404, detail: `Session ${sid} not found` };
  ...
  finalize: async (sid: string, _token: string) => {
    ...
    await runTransaction(ref(db, `auctions/${sid}`), (doc) => {
      ...
      const bids = Object.entries(d.currentBids)
        ...
        .filter((x) => x.bid > 0);
      if (!bids.length) {
        txnError = "No bids. Use Skip instead.";
        return;
      }
      ...
      if (winners.length > 1) {
        const names = winners
          .map((w) => d.teams.find((t: any) => t.id === w.teamId)?.name)
          .join(", ");
      ...
      if (win.players.length >= TEAM_SIZE) {
        txnError = `${win.name} is full`;
        return;
      }
      if (highest > win.budget) {
        txnError = `${win.name} cannot afford`;
        return;
      }
  ...
  skip: async (sid: string, _token: string) => {
    ...
    await runTransaction(ref(db, `auctions/${sid}`), (doc) => {
      ...
      const moved = {
        ...pool[effPlayer],
        isRetry: true,
        retryCount: (pool[effPlayer].retryCount ?? 0) + 1,
      };
      pool.splice(effPlayer, 1);
      pool.push(moved);
      d.playerPools[poolKey!] = pool;
